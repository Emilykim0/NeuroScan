...
def train_model(model, train_loader, val_loader, criterion, optimizer, device, num_epochs=100, patience=7):
    best_acc = 0.0
    ...
    for epoch in range(1, num_epochs+1):
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            torch.save(model.state_dict(), f"./saved_models/model_acc_{best_acc:.4f}.pt")
            with open(f"./saved_models/cm_acc_{best_acc:.4f}.pkl", "wb") as f:
                pickle.dump(cm, f)
            print("Best model and confusion matrix saved!\n")
